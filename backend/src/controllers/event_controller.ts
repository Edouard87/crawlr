...
export const deleteEvent = async (req: Request, res: Response) => {
  ...
  if (!event) {
    throw new Error.DocumentNotFoundError('Could not find event with id ' + req.params.id);
  }
  
  res.status(200).json({ message: "Event deleted successfully" });
};

export const getEventByCoordCode = async (req: Request, res: Response) => {
  const event = await EventService.getEventByCode(req.params.coordCode)
  res.status(200).json(event);
}
